...
// تحميل عروض الباقات
function loadBundleOffers() {
    const bundleProducts = document.getElementById('bundleProducts');
    if (!bundleProducts) return;

    const bundleItems = products.filter(product => product.category === 'bundle');
    
    if (bundleItems.length === 0) {
        bundleProducts.innerHTML = '<p>No bundle offers available yet.</p>';
        return;
    }

    bundleProducts.innerHTML = bundleItems.map(product => `
        <div class="product-card" onclick="showProductDetail('${product.id}')">
...
// تفعيل زر Create Bundle
document.querySelector('.create-bundle').addEventListener('click', function() {
    showPage('bundle');
    loadBundleOffers();
});
// إظهار الصفحات
